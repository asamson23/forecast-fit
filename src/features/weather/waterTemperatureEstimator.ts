...
export type WaterModelSettings = {
  waterBodyType?: WaterBodyType | string;
  windExposure?: WindExposureType | string;
  poolType?: string;
};

export type WaterEstimateInput = {
...
export type WaterEstimateResult = {
  available: boolean;
  source: 'estimated' | 'unknown';
  confidence: 'medium' | 'low' | 'unknown';
  temp?: number;
  conservativeTemp?: number;
  rangeLow?: number;
  rangeHigh?: number;
  explanation: string;
...
type LatitudeBand = {
  key: 'tropical' | 'warm' | 'temperate' | 'cold' | 'subpolar';
  label: string;
  bias: number;
};
...
export function clampEstimate(value: number, min: number, max: number): number {
...
export function getLatitudeBand(latitude: unknown): LatitudeBand {
...
export function getSeasonInfo(dateStr: unknown, latitude: unknown): SeasonInfo {
  const date = parseLocalString(String(dateStr || '').slice(0, 10) + 'T12:00') || new Date();
  const month = date.getMonth();
  const day = date.getDate();
  const north = (firstFinite(latitude, 45) ?? 45) >= 0;
  const seasonsNorth: SeasonInfo['season'][] = ['winter', 'winter', 'spring', 'spring', 'spring', 'summer', 'summer', 'summer', 'autumn', 'autumn', 'autumn', 'winter'];
  const seasonsSouth: SeasonInfo['season'][] = ['summer', 'summer', 'autumn', 'autumn', 'autumn', 'winter', 'winter', 'winter', 'spring', 'spring', 'spring', 'summer'];
  const season = (north ? seasonsNorth : seasonsSouth)[month] || 'summer';
  const phase: SeasonInfo['phase'] = day <= 10 ? 'early' : day <= 20 ? 'mid' : 'late';
  const autumnStart = new Date(date.getFullYear(), north ? 8 : 2, 1, 12, 0, 0, 0);
  let weeksIntoAutumn = 0;
  if (season === 'autumn') weeksIntoAutumn = Math.max(0, (date.getTime() - autumnStart.getTime()) / (7 * 24 * 3600 * 1000));
  const factor = (
    season === 'spring' ? 0
...
export function getWaterBodyConfig(type: unknown): WaterBodyConfig {
  const config = ({
  ...
  } as Record<string, WaterBodyConfig>)[String(type || 'auto')];
  return config || { label: 'unknown water body', depthFactor: 0.5, positiveScale: 1, adjust: 0 };
}

export function getRecentDailyRecordsForWater(data: WaterEstimateInput): NonNullable<WaterEstimateInput['daily']> {
...
export function getRecentHourlyRecordsForWater(data: WaterEstimateInput): NonNullable<WaterEstimateInput['hourly']> {
...
export function getAlignedDailyWaterRecords(data: WaterEstimateInput): DailyWaterRecord[] {
...
export function getRecentWindWindow(data: WaterEstimateInput, hours = 24) {
  const currentMs = parseAnyTime(data?.currentTime);
  const hourly = Array.isArray(data?.hourly) ? data.hourly : [];
...
export function getSeasonalDepthEffect(season: SeasonInfo, body: WaterBodyConfig): number {
...
export function getSpringCap(band: LatitudeBand, waterBodyType: unknown): number {
...
export function getSummerFloor(band: LatitudeBand, waterBodyType: unknown): number | null {
  if (!['shallow_lake', 'bay', 'sheltered'].includes(String(waterBodyType || ''))) return null;
  return band.key === 'tropical' ? 22 : band.key === 'warm' ? 20 : band.key === 'temperate' ? 18 : 16;
}

export function getDynamicUncertainty(args: {
  confidence: WaterEstimateResult['confidence'];
  settings: WaterModelSettings;
  season: SeasonInfo;
  lows: number[];
  highs: number[];
  recentWindHours: Array<{ wind?: number | null }>;
}): number {
  let uncertainty = args.confidence === 'medium' ? 2.0 : 2.75;
  if (String(args.settings.waterBodyType || 'auto') === 'auto') uncertainty += 0.5;
  if (String(args.settings.windExposure || 'auto') === 'auto') uncertainty += 0.25;
  if (args.season.season === 'winter') uncertainty += 0.75;
  else if (args.season.season === 'spring') uncertainty += 0.35;
  if (args.lows.length < 5 || args.highs.length < 5) uncertainty += 0.5;
  if (args.recentWindHours.length < 12) uncertainty += 0.25;
  return clampNumber(uncertainty, 2.0, 4.25);
}

function getWaterBodyDefinition(type: unknown) {
  return WATER_BODY_TYPE_DEFINITIONS.find((entry) => entry.key === String(type || 'auto')) || WATER_BODY_TYPE_DEFINITIONS[0];
}

function getWindExposureDefinition(type: unknown) {
  return WIND_EXPOSURE_DEFINITIONS.find((entry) => entry.key === String(type || 'auto')) || WIND_EXPOSURE_DEFINITIONS[0];
}

export function estimatePseudoWaterTemperature(data: WaterEstimateInput, settings: WaterModelSettings = {}): WaterEstimateResult {
  ...
  const windyHours = recentWindHours.filter((point) => firstFinite(point?.wind, null) != null && (firstFinite(point?.wind, null) as number) >= windyWindKmh);
  ...
  const confidence: WaterEstimateResult['confidence'] =
    hasEnoughDailyData && hasEnoughWindData && hasManualBodyType && hasUsefulWindExposure && isNotWinter
      ? 'medium'
      : 'low';
