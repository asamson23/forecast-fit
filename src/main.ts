...
// Pool swim is split in the UI, but both versions share the same distance/session presets.
eventPresetsByActivity.swimming_pool_indoor = eventPresetsByActivity.swimming_pool.map(p => ({
  ...p,
  key: `indoor_${p.key}`,
  detail: p.detail.replace('pool', 'indoor pool')
}));
eventPresetsByActivity.swimming_pool_outdoor = eventPresetsByActivity.swimming_pool.map(p => ({
  ...p,
  key: `outdoor_${p.key}`,
  detail: p.detail.replace('pool', 'outdoor pool')
}));

const distanceUnitOptionsByActivity = {
...
const averageUnitOptionsByActivity = {
...
const avgDeriveActivities = new Set(['running', 'cycling', 'swimming_open', 'swimming_pool', 'swimming_pool_indoor', 'swimming_pool_outdoor', 'gym', 'indoor_running', 'indoor_cycling', 'indoor_multisport', 'sup', 'kayaking', 'snorkeling', 'water_sports', 'hiking', 'trail_running', 'mtb_gravel', 'ski_snowboard', 'road_trip', 'walk', 'fishing', 'hunting', 'casual']);
...
function ecccFeatureContainsPoint(feature, lat, lon) {
  ...
  if (geom.type === 'Polygon') {
    ...
    if (!rings.length || !pointInRing(lo, la, rings[0])) return false;
    return !rings.slice(1).some(ring => pointInRing(lo, la, ring));
  }
  if (geom.type === 'MultiPolygon') {
    return (geom.coordinates || []).some(poly => {
      const rings = poly || [];
      return rings.length && pointInRing(lo, la, rings[0]) && !rings.slice(1).some(ring => pointInRing(lo, la, ring));
    });
  }
  return false;
}
...
async function fetchEcccWeatherAlertsForPoint(lat, lon, countryCode = '') {
  ...
  try {
    ...
    const alerts = (json.features || [])
      .filter(feature => isActiveEcccAlertFeature(feature, now))
      .filter(feature => ecccFeatureContainsPoint(feature, lat, lon))
    ...
    return { source: 'eccc', status: 'error', alerts: [], error: error?.message || String(error) };
  }
}
...
async function searchPlaces(query, count = 6) {
  ...
  try {
    ...
    if (Array.isArray(geoData.results)) results.push(...geoData.results.map(r => normalizeSearchResult(r, 'openmeteo')));
  } catch (_) {}
  try {
    ...
    if (Array.isArray(nomData)) results.push(...nomData.map(r => normalizeSearchResult(r, 'nominatim')));
  } catch (_) {}
...
function getBestWindowCondenseMinutes(stepMinutes, durationMinutes) {
  ...
  // Nearby starts usually mean the same real-world outing. Scale the merge
  // distance with duration, but keep it bounded so short workouts still show
  // meaningful alternatives and long outings do not spam near-duplicates.
...
function clusterBestWindowCandidates(candidates, stepMinutes, maxWindows = 6, minWindows = 3, durationMinutes = getBestWindowDurationMinutes()) {
  const valid = candidates
    .filter(c => c.valid)
    .sort((a, b) => parseAnyTime(a.startTime) - parseAnyTime(b.startTime));
  if (!valid.length) return [];

  const bestScore = Math.max(...valid.map(c => firstFinite(c.score, -Infinity)));
  const scoreFloor = Math.max(42, bestScore - 18);
  const condenseMinutes = getBestWindowCondenseMinutes(stepMinutes, durationMinutes);

  function buildClusters(source) {
    const clusters = [];
    for (const candidate of source) {
      const candidateMs = parseAnyTime(candidate.startTime);
      ...
      const gapMin = Math.abs((candidateMs - parseAnyTime(lastItem.startTime)) / 60000);
      const repGapMin = Math.abs((candidateMs - parseAnyTime(previous.bestCandidate.startTime)) / 60000);
      ...
      if ((gapMin <= condenseMinutes || repGapMin <= condenseMinutes) && scoreDelta <= 14) {
        ...
        if (firstFinite(candidate.score, -Infinity) > firstFinite(previous.bestCandidate.score, -Infinity)) previous.bestCandidate = candidate;
      } else {
        clusters.push({ items: [candidate], bestCandidate: candidate, start: candidate.startTime, end: candidate.startTime });
      }
    }
    return clusters.map(rankBestWindowCluster).sort((a, b) => b.score - a.score);
  }

  let ranked = buildClusters(valid.filter(c => firstFinite(c.score, -Infinity) >= scoreFloor));
  if (ranked.length < Math.min(minWindows, maxWindows)) {
    ranked = buildClusters(valid);
  }

  const selected = [];
  for (const cluster of ranked) {
    const repMs = parseAnyTime(cluster.representative?.startTime);
    const tooClose = selected.some(existing => {
      const existingMs = parseAnyTime(existing.representative?.startTime);
      return Math.abs(repMs - existingMs) / 60000 < condenseMinutes * 0.85;
    });
    if (tooClose && selected.length >= Math.min(minWindows, ranked.length, maxWindows)) continue;
    selected.push(cluster);
    if (selected.length >= maxWindows) break;
  }

  addMinimumBestWindowFallbacks(selected, valid, Math.min(minWindows, maxWindows, valid.length), maxWindows, stepMinutes, condenseMinutes, durationMinutes);
  return selected
    .sort((a, b) => firstFinite(b.score, -Infinity) - firstFinite(a.score, -Infinity))
    .slice(0, maxWindows)
...
function addMinimumBestWindowFallbacks(selected, validCandidates, minWindows, maxWindows, stepMinutes, condenseMinutes, durationMinutes) {
  ...
  function tryAddWithMinSeparation(minSeparation) {
...
document.addEventListener('keydown', event => {
  if (event.key === 'Escape' && quickStartOverlay && !quickStartOverlay.hidden) closeQuickStartGuide();
});
